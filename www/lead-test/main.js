...
! function(e) {
    e(".slider-1").slider({
        ...
        step: 100,
        slide: function(t, a) {
          e('.audience').text(a.value);
            e(".slider-1 .ui-slider-handle").attr("data-value", a.value);
    ...
    }), e(".slider-1 .ui-slider-handle").attr("data-value", 5e3);
...
e(".slider-2").slider({
        ...
        step: 1,
        slide: function(t, a) {
            e(".slider-2 .ui-slider-handle").attr("data-value", a.value);
            var r = e(".slider-2").find(".slider-2 .ui-slider-handle");
            r.css("margin-left", -1 * r.width() * (e(".slider-2").slider("value") / e(".slider-2").slider("option", "max")))
        }
    }), e(".slider-2 .ui-slider-handle").attr("data-value", 72);

interest        = parseFloat($('.per').text().replace(/[^0-9.]/g, ""));
  function calculate() {
    var ctavka = interest/100;
    var summa = parseInt($('.audience').text());
    var percent = parseInt(summa) * parseInt(ctavka);
    var vuvod = parseInt(percent) + parseInt(summa);

    console.log(summa);
    $('.otd').html(parseFloat(vuvod).toFixed(2));
